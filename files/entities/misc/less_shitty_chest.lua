...
local function get_rewards(x,y,entityid)
	...
	if rng==1 then --roll additional loot
	...
	elseif rng==5 or rng==6 then --pouch/potion
		...
		rng=Random(1,10)
		if rng==1 then --truly random material (10%)
		...
		elseif rng<=4 then -- alchemy (30%)
			...
			selectedmaterial=allmat[Random(1,#allmat)]
		elseif rng<=7 then --magic_liquid (30%)
	...
	elseif rng==9 or rng==10 then --misc item
		...
		if rng==1 then --elemental stone
			if Random(1,10)==1 then --10% for poopstone
		...
		elseif rng==2  then --runestone
			...
			if ModIsEnabled("bags_of_many") then
				if Random(1,3)==1 then
					...
					p[#p+1]="mods/bags_of_many/files/entities/bags/bag_universal_small.xml"
					p[#p+1]="mods/bags_of_many/files/entities/bags/bag_potions_small.xml"
				end
			end
