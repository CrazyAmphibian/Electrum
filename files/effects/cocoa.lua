...
function damage_about_to_be_received( damage, x, y, ent_thats_responsible, crit_chance )
	...
	mult=mult*(tonumber( ComponentGetValue2( damagemodel, "hp" ) )-damage)/tonumber( ComponentGetValue2( damagemodel, "max_hp"))<=.5 and .75 or 1 --20% to 40% damage reduction under 1/2 health. operates like the brass beast from TF2, so if the hit would reduce you to that amount.
