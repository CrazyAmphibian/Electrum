...
if invcomp then
	...
	if highestid and matcount>1 then --ensure that it doesn't delete everything.
		local newent=EntityLoad("mods/Electrum/files/entities/items/disposablepotion.xml",x+math.random(-10,10),y+math.random(3,5) )
		AddMaterialInventoryMaterial(newent, CellFactory_GetName(highestid-1) ,highestcount) 
		AddMaterialInventoryMaterial(ents[i], CellFactory_GetName(highestid-1) ,0) 
	end
