...
local function lists_share_all_members(list1,list2)
	if #list1~=#list2 then return false end
	for i=1,#list1 do
		if not member_in_list(list2,list1[i]) then return false end
	end
	return true
end
...
local function transmute(mat_name)
	...
	if database[mat_name] then
		...
		for i=1,#tags do --for each tag our material has...
			...
			for n=1,#subset do --add every other material with that tag.
				local thismat=subset[n]
				if (not member_in_list(eligable,thismat) ) then
